<head>{literal}
    ...
    <style>
        ...
        *{-webkit-box-sizing:border-box;-moz-box-sizing:border-box;box-sizing:border-box;}
        ...
        .loader{position:absolute; display: none; width:100%; height:100%; background-color:grey; opacity: 0.3;z-index: 4;}
        .loader-icon{position:absolute;display: none; background:url('{/literal}{$SLY_LOGOS_PATH}{literal}loading-icon.svg') no-repeat;z-index: 5;width: 100px; height:100px;left:50%;top:500px; }
...
<body>
<div class="container-fluid mar-top-btt-80 " id="mymodule_wrapper" data-token="{$token}">
    <div class="loader" id="ajaxsearchloader"></div><div class="loader-base"><span class="loader-icon"></span></div>
    <div class="row">
        <div class="col-lg-2 col-md-4 col-sm-12 pad-10">
            ...
            <select class="btn btn-mini" name="debugmode" onchange="ajaxexecuter(this);">
                 {$SLY_DEBUGMODE_SELECT}
            </select>
            </div>
            </div>
        </div>
    ...
    <div class="row mar-top-btt-80">
        <div class="col-lg-4 col-md-4 col-sm-12">
            <div class="w-100 btn-group btn-group-toggle">
                ...
                <button type="button" class="btn btn-success btn-xs" title="Reload Logs" onclick="ajaxexecuter(this);" name="showlogfiles"><i class="fas fa-sync-alt"></i> Reload logs</button>
                <button type="button" class="btn btn-danger btn-xs" title="Delete logs" onclick="ajaxexecuter(this);" name="deletelogfile"><i class="fas fa-trash-alt"></i> Remove logs</button>
            </div>
...
<script>
    ...
    function sync_custom_command(command,value) {
        if(document.getElementById('ajaxonline').value === '0'){
            ...
            if(command != 'showlogfilecontent' && command != 'debugmode' ){
                value = document.getElementById('statline').value;
            }
            jQuery.ajax({
                method: "POST",
                url: "{/literal}{$diag_link}{literal}",
    ...
    function files_for_delete(command) {
        if(document.getElementById('ajaxonline').value === '0'){
            ...
            jQuery.ajax({
                method: "POST",
                url: "{/literal}{$delete_link}{literal}",
    ...
    document.addEventListener('DOMContentLoaded',function (){ sync_custom_command('showlogfiles','');}, false);
</script>{/literal}
</body>