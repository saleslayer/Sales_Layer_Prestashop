{* Load Stylesheet Files *}{strip}
    ...
    <div class="container" id="mymodule_wrapper" data-token="{$token}">
        <input type="hidden" id="ajax_link_sl" value="{$ajax_link}" />
        <input type="hidden" id="allelements" value="0" />
        <div class="row mar-top-btt-40 slyr-import-ajax-form slyr-import-form-login" >
            <div class="col-md-4" id="logo">
                <img src="{$SLY_LOGOS_PATH}logob_{$COMPANY_TYPE}.png" height="65px;" alt="logo slyr">
            </div>
            <div class="col-md-4 slyr-form-field-block">
                <a href="{$link_all_connectors}" class="btn btn-success width-150 mar-top-btt-10"><i class="fa fa-plus text-left" aria-hidden="true"></i> New connector</a>
            </div>
            <div class="col-md-4 slyr-form-field-block">
                 {$purge_button}{$stop_syncronization}
            </div>

        </div>
        <div class="row" id="progressbar"></div>
        <div class="row mar-top-btt-40" id="slyr-import-module-block">
            <div class="mar-top-btt-10 mar-top-btt-40"><h1>{$COMPANY_NAME} - Update Categories &amp; Products</h1></div>
            <div  class="row text-center"><span id="messages" >{$messages}</span></div>
    ...
    <script>
        ...
        function update_conn_field(data) {
            ...
            jQuery.ajax({
                ...
                dataType: "json",
                data: {'connector_id': connector_id, 'field_name': field_name, 'field_value': field_value ,'token':token},
