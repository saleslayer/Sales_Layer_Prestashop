...
class saleslayerimportajaxModuleFrontController extends ModuleFrontController
{
    ...
    public function displayAjax()
    {
       ...
       if(substr(Tools::encrypt('saleslayerimport'), 0, 10) !=  Tools::getValue('token')){
        ...
        if($command == 'check_status') {
            ...
            $sql_processing = "SELECT count(*) as sl_cuenta_registros FROM ".$SLimport->saleslayer_syncdata_table;
            $items_processing = $SLimport->sl_connection_query('read', $sql_processing);
            ...
            if (isset($items_processing['sl_cuenta_registros']) && $items_processing['sl_cuenta_registros'] > 0){
                ...
                if(count($result) && $register_forProcess) {
                  ...
                  //  $old_execution_time = $SLimport->checkCronProcessExecutionTime();
                   // $SLimport->debbug('old Latest executed time of cron ->    ' . date('H:i:s',$old_execution_time) . 'Latest updated time in bd ->'.date('H:i:s',$updated_time) ,'syncdata' );
                   ...
                   // $SLimport->debbug('Latest Execution time of cron is  ' . date('d-m-Y H:i:s',$updated_time) . 'next start in -> ' . $this->timeOccurence( $start_in_bd_seconds).' at server time ->'.date('d-m-Y H:i:s',$start_at_servertime) , 'syncdata');
        ...
        if($command == 'delete_now') {
            ...
            $resultDelete = $SLimport->sl_updater->delete_connector($connector_id, true);
        ...
        if($command == 'clear_syncronization') {
            ...
            $SLimport = new SalesLayerImport();
            $sql_processing = "DELETE FROM ".$SLimport->saleslayer_syncdata_table;
            $items_processing = $SLimport->sl_connection_query('-', $sql_processing);
      ...
      if($connector_id != null){
          ...
          if(in_array($field_name,$permited_fields,false)){
                ...
                try{
                    ...
                    if(($field_name == 'auto_sync' && ( $field_value >= 0 && $field_value <= 72 ) )|| ( $field_name == 'auto_sync_hour' && ($field_value >= 0 && $field_value <= 24 )) || ($field_name == 'avoid_stock_update' && ($field_value == 1 || $field_value == 0 ))){

                        $shops_info = $SLimport->set_connector_Data($connector_id,$field_name,$field_value);
                    ...
                    $return['message_type'] = 'error';
                    $return['message']      = 'An error occurred in saving changes in the connector.';
                    $SLimport->debbug('An error occurred in saving changes in the connector '.print_r($e->getMessage(),1));
                }
              ...
              if($field_name != null){
                  ...
                  if($sting_toarray[0] == 'shops'){
                       ...
                       */
                        ...
                        try{
                            $conn_extra_info = $SLimport->sl_updater->get_connector_extra_info($connector_id);
                            $need_save = false;
                            ...
                            if($need_save){
                                $SLimport->sl_updater->set_connector_extra_info($connector_id, $conn_extra_info);
                                $return['message_type'] = 'success';
                                $return['message']      = 'Changes saved successfully.';
                            }else{
    ...
    private function purgeAllElementsFromPresta(){
        ...
        $SLimport->reasignMemoryNeccesary();
        ...
        $res = Db::getInstance()->executeS('SELECT `id_category` FROM `'._DB_PREFIX_.'category` WHERE id_category != "0" AND is_root_category = "0" ');
        $SLimport->reasignMemoryNeccesary();
        if ($res) {
            foreach ($res as $row) {
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                Db::getInstance()->execute(sprintf("DELETE FROM ".$SLimport->image_table." WHERE id_image = '%s' ",$row['id_image']));
                Db::getInstance()->execute(sprintf("DELETE FROM ".$SLimport->image_lang_table." WHERE id_image = '%s' ",$row['id_image']));
                Db::getInstance()->execute(sprintf("DELETE FROM ".$SLimport->image_shop_table." WHERE id_image = '%s' ",$row['id_image']));
                Db::getInstance()->execute(sprintf("DELETE FROM ".$SLimport->slyr_images_table." WHERE id_image = '%s' ",$row['id_image']));
        ...
        Db::getInstance()->execute("DELETE FROM ".$SLimport->slyr_table);

        Shop::setContext(Shop::CONTEXT_SHOP,$contextShopID);

    }

    public function updateConector($conn_code){
        $SLimport = new SalesLayerImport();
        $return = '';
            ...
            if ($SLimport->checkRegistersForProccess()){
                ...
                try{
                   $response =  $sync_libs->sync_data_connectors();
                    ...
                    $SLimport->debbug('## Error. Sync data connectors '.$conn_code.' : '.$e->getMessage().' line->'.$$e->getLine(),'error');
                    $return = 'Error: Sync data connectors  '.$conn_code.' : '.$e->getMessage();
                    $update_success = false;
                }
            }else{
                try{
                    $returnUpdate = $sync_libs->store_sync_data($conn_code);
                    ...
                    if(is_array($returnUpdate)  ){
                        if(count($returnUpdate)){
                            ...
                            if(isset($returnUpdate['product_formats_to_delete'])&&$returnUpdate['product_formats_to_delete']> 0){
                            ...
                            if(isset($returnUpdate['product_formats_to_sync']) && $returnUpdate['product_formats_to_sync']> 0){
                    ...
                    $SLimport->debbug('## Error. In store data '.$conn_code.' : '.$e->getMessage().' trace->'.print_r($e->getTrace(),1).' line->'.$e->getLine());
