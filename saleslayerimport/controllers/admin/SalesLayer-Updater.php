        ...
        if (in_array($sly_table, $this->database_tables)) {
                ...
                foreach ($fields as $name => $field) {
                    ...
                    if (   isset($schema[$field])) {
                        ...
                        if ($force_default_language && $multi && $language != $base_language) {
                            ...
                            $select .= ($select ? ', ' : '').
                                       "IF(`$this_db_table`.`$db_field`!='', `$this_db_table`.`$db_field`, `$this_db_table_base`.`$this_db_table_base`) as `$field_name`";
            ...
            if ($select) {
                    ...
                    foreach ($conditions as &$param) {
                        ...
                        if (isset($param['group'])) {
                            ...
                            if  (isset($param['search']) && $param['search']) {
                                ...
                                $this_db_table = $this->__get_table_for_field($db_field, $table);
                                $clause   = "`$this_db_table`.`$db_field`".(($param['condition']) ? $param['condition'] : '=')."'".addslashes($param['value'])."'";
                                ...
                                if (   $force_default_language
                                    ...
                                    $this_db_table = $this->__get_table_for_field($db_field, $table);
                                    $clause        = "($clause or `$this_db_table`.`$db_field`".($param['condition'] ? $param['condition'] : '=')."'".addslashes($param['value'])."')";
                ...
                if ($string_tables) {
                    ...
                    $SQL = 'select '.($get_internal_ids ? "`$field_id` as `$field_id_name`, " : '').($get_channel_id ? '`__conn_id__`, ' : '')."$select from $string_tables".
