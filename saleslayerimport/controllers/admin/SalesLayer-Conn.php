...
class SalesLayerConn
{
    ...
    protected $__get_force_update_only                  = false;
    ...
    private function hasSystemRequirements()
    {
        if (!extension_loaded('curl')) {
            $this->triggerError('Missing PHP curl extension', 100);
     ...
     * @return string
     */
    private function getApiUrl($last_update = false)
    {
        ...
        if (false !== $this->__get_force_update_only) {
            $URL .= '&force_update_only=1';
        }
     ...
     */
    public function setIdentification($codeConn, $secretKey = null)
    {
     ...
     */
    public function setSslConnection($stat)
    {
        $this->SSL = $stat;
    }
     ...
     */
    public function setSslCredentials($cert = null, $key = null, $CACert = null)
    {
        $this->SSL_Cert   = $cert;
        $this->SSL_Key    = $key;
     ...
     */
    public function setUrlConnection($url)
    {
        if ($url) {
            $this->url = $url;
        }
     ...
     * @param bool $group
     */
    public function setGroupMulticategory($enable)
    {
     ...
     * @param bool $enable
     */
    public function setSameParentVariantsModifications($enable)
    {
     ...
     * @param bool $enable
     */
    public function setParentsCategoryTree($enable)
    {
        $this->__get_parents_category_tree = $enable;
    }

    public function setForceUpdateOnly($enable)
    {
        $this->__get_force_update_only = $enable;
    }
     ...
     */
    public function getInfo($last_update = null, $params = null, $connector_type = null, $add_reference_files = false)
    {
        if ($this->hasConnector()) {
            ...
            $stat = $this->call($this->getApiUrl($last_update), $params);
     ...
     */
    public function setInfo($update_items = [], $delete_items = [], $compression = false, $force_directly = false, $extra_params = [])
    {
        $params = [];
        ...
        if ($this->hasConnector()) {
            ...
            if (!empty($params)) {
                ...
                $stat = $this->call($this->getApiUrl(), $params);
     ...
     * @return bool
     */
    public function haveInputErrors()
    {
     ...
     * @return bool
     */
    public function haveInputWarnings()
    {
    ...
    public function checkInputTracking()
    {
        if (!empty($this->response_input_tracking) && time() > $this->resonpse_last_time_check) {
            ...
            $this->response_input_tracking_message = '';
        } elseif (empty($this->response_input_tracking)
                   &&       $this->response_input_tracking_status != 'error'
    ...
    public function getInputTrackingStatus()
    {
        $this->checkInputTracking();
    ...
    public function getInputTrackingPercent()
    {
        $this->checkInputTracking();
    ...
    public function getInputTrackingMessage()
    {
        $this->checkInputTracking();
        ...
        if ($url and preg_match('/^https?:\/\/'.preg_quote(preg_replace('/^([^\/?]+[\/?]).*$/', '\\1', $this->url), '/').'/i', $url)) {
            ...
            if (false !== $response) {
                ...
                if (false !== $this->data_returned && is_array($this->data_returned)) {
                    ...
                    $this->triggerError('Void response or malformed: ' . $response, 101);
                }
            } else {
                $this->triggerError('Error connection: ' . curl_error($ch), 102);
            }

            curl_close($ch);
        } else {
            $this->triggerError('Incorrect URL call: ' . $url, 100);
        }
        ...
        if (null !== $this->data_returned) {
            ...
            if (isset($this->data_returned['error']) && $this->data_returned['error']) {
                if (isset($this->__error_list[$this->data_returned['error']])) {
                 ...
                 $this->triggerError($message_error, $this->data_returned['error']);
            } else {
                $status = false;
     ...
     */
    public function triggerError($message, $errnum)
    {
     ...
     * @return int
     */
    public function getResponseTime($mode = 'datetime')
    {
     ...
     * @return array
     */
    public function getResponseTableInformation($table = null)
    {
     ...
     * @return array
     */
    public function getResponseTableData($table = null)
    {
     ...
     * @return array
     */
    public function getResponseTableDeletedIds($table = null)
    {
     ...
     * @return array
     */
    public function getResponseTableModifiedIds($table = null)
    {
     ...
     * @return array
     */
    public function getResponseTableModifiedData($table = null)
    {
        if (null === $table) {
     ...
     * @return array
     */
    public function getSchemaInformation()
    {
        if (isset($this->data_returned['data_schema_info'])) {
     ...
     * @return array
     */
    public function getResponseFieldTitles($table = null)
    {
        $titles = [];
     ...
     * @return array
     */
    public function getResponseLanguageFieldTitles($language, $table = null)
    {
        $titles = [];
    ...
    public function getHeadings($language, $table = null)
    {
            ...
            if (isset($this->response_headings[$table])) {
                foreach ($this->response_headings[$table] as $info) {
                    ...
                    if (isset($info['titles'])) {
                        if (isset($info['titles'][$language])         and $info['titles'][$language]) {
                            $title = $info['titles'][$language];
                        } elseif (isset($info['titles'][$default_language]) and $info['titles'][$default_language]) {
                            $title = $info['titles'][$default_language];
                        }
                    }
