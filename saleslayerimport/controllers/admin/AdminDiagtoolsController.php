...
class AdminDiagtoolsController extends ModuleAdminController
{
    ...
    public function generateServerInfo()
    {
        ...
        $array_toshow = ['Server Software', isset($_SERVER['SERVER_SOFTWARE']) ? $_SERVER['SERVER_SOFTWARE'] : 'N/A'];
        $this->formatTable($array_toshow);

     /*   $array_toshow = ['intl.error_level',ini_get('intl.error_level')];
        $this->formatTable($array_toshow);*/

        $array_toshow = ['Server name', isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : 'N/A'];
        $this->formatTable($array_toshow);

        $array_toshow = ['Server http_host', isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : 'N/A'];
        $this->formatTable($array_toshow);
        ...
        // Verificar que $free_memory es un array válido antes de acceder a sus índices
        $free_memory_mb = 0;
        if (is_array($free_memory) && isset($free_memory['frmem'])) {
            $free_memory_mb = $free_memory['frmem'];
        }
        ...
        if ($max_memory_usage) {
            ...
            if ($free_memory_mb > 0 && $max_memory_usage > $free_memory_mb) {
                $class = "text-danger";
                $text = 'For synchronization you needed more memory than is available now.';
            } elseif ($free_memory_mb > 0) {
                $class = "text-success";
                $text = 'There is supposedly enough memory to receive data.';
            }
        }

        // Mostrar memoria libre solo si está disponible
        if ($free_memory_mb > 0) {
            $array_toshow = ['Free memory','<span class="' . $class . '" title="' . $text . '">' .
                                           $free_memory_mb . ' Mb</span>'];
        } else {
            $array_toshow = ['Free memory','<span class="text-warning" title="Memory information not available">' .
                                           'N/A (Alpine Linux or /proc/meminfo not accessible)</span>'];
        }
        $this->formatTable($array_toshow);
