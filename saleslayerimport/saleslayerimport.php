        ...
        if (!$force_print) {
            ...
            if (!$this->debugmode && $error_content && !$force_print) {
                ...
                if (count($this->debug_occurence) > 20) {
                    $actual_limit = $this->parseMemoryLimit(ini_get('memory_limit') ?? '320M');
...
FROM ' . $this->prestashop_cron_table . $where . ' LIMIT 1';
    ...
    /**
     * Parse memory limit string to MB integer
     * Converts "512M", "1G", "2048K" to MB value
     *
    ...
    public function allocateMemory()
    {
        ...
        $actual_limit_str = ini_get('memory_limit');
        $actual_limit = $this->parseMemoryLimit($actual_limit_str);
