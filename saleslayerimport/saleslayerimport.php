...
require 'controllers/admin/SalesLayer-Conn.php';
require 'controllers/admin/SalesLayer-Updater.php';
if (extension_loaded('PDO')) { if (!class_exists('slyr_SQL'))        include_once dirname(__FILE__) . DIRECTORY_SEPARATOR . 'controllers/admin/lib/class.DBPDO.php'; }
else if                           (!class_exists('slyr_SQL'))        include_once dirname(__FILE__) . DIRECTORY_SEPARATOR . 'controllers/admin/lib/class.MySQL.php';
...
class SalesLayerImport extends Module {
    ...
    public      $slyr_table                          = _DB_PREFIX_.'slyr_category_products';
    ...
    public      $deleteProductOnHide                 = false;  // only for debug,  force delete products from prestashop if is a false == deactivate product, true == delete product
    private     $memory_min_limit                    = 200;    // set minimal memory needed
    private     $sql_insert_limit                    = 5;      // number of items that will be used per query in synchronization
    ...
    private     $debbug_occurence                    = array();
    ...
    public      $product_format_base_fields          = array('ID', 'ID_products', 'frmt_image', 'location', 'ean13', 'upc', 'quantity', 'reference', 'wholesale_price', 'price', 'price_tax_incl', 'price_tax_excl', 'ecotax', 'weight', 'unit_price_impact', 'minimal_quantity',
                                                             'default_on', 'available_date', 'mostrar', 'format_images', 'format_supplier', 'format_supplier_reference');

    public      $predefined_product_fields           = array('product_type','product_name','product_reference','product_ean13','product_upc','product_active','product_visibility','product_available_for_order','product_show_price','product_available_online_only',
                                                            'product_condition','product_description_short','product_description','product_price','product_id_tax_rules_group','product_price_tax_incl','product_discount_1','product_discount_1_type','product_discount_1_quantity',
                                                            'product_discount_2','product_discount_2_type','product_discount_2_quantity','meta_title','meta_description','friendly_url','category_default','product_accessories','product_manufacturer',
                                                            'product_width','product_height','product_depth','product_weight','additional_shipping_cost','product_carrier','product_quantity','product_minimal_quantity','product_out_of_stock','available_now',
                                                            'available_later','product_image','product_available_date','product_creation_date','product_customizable','pack_product_1','pack_format_1','pack_quantity_1','product_supplier_1','product_supplier_reference_1');
    ...
    public function __construct(){
        ...
        require_once 'controllers/admin/sl_catalogues.php';
        require_once 'controllers/admin/sl_products.php';
        require_once 'controllers/admin/sl_variants.php';

        $this->name                   = 'saleslayerimport';
        $this->tab                    = 'administration';
        $this->version                = '1.4';
        $this->author                 = COMPANY_NAME;
        $this->connector_type         = 'CN_PRSHP';
        ...
        $this->displayName = $this->l(COMPANY_NAME.' Import');
        $this->description = $this->l('Import products from '.COMPANY_NAME.' API.');
        $this->confirmUninstall = $this->l('Are you sure you want to uninstall?');
        $this->sl_time_ini_process = microtime(1);
        $this->sl_updater   = new SalesLayer_Updater(_DB_NAME_,_DB_USER_,_DB_PASSWD_,_DB_SERVER_);
        $this->sl_updater->set_table_prefix(_DB_PREFIX_.'slyr_');
    ...
    /**
     * Recalculate the maximum time allowed for this synchronization by checking the frequency of cron calls and the maximum allowed time of execution of cron of php
     * @throws PrestaShopDatabaseException
     * @throws PrestaShopException
     */
    private function recalculateDurationOfSyncronizationProcess(){
        if($this->rewrite_execution_frequency){
            ...
            if($actual_max_execution_limit > 0){ // no tiene limite
                $actual_execution_limit =  ( $actual_max_execution_limit - 10 );// -10 seconds for the process to end before the other was executed and you have 10 seconds of reservation to finish any process you are doing.
            ...
            if($actual_execution_limit  > 0 && $actual_execution_limit <= $execution_time_cron ){
                ...
                $this->debbug('Set Max execution time from register of ini_get max_execution_time '.$transcured,'syncdata');
            ...
            if($execution_time_cron  > 0 && $actual_execution_limit >= $execution_time_cron ){
                ...
                if(count($result)) {
                    ...
                    $this->debbug('It remains seconds until executing another cron '.print_r($restand_seconds_for,1),'syncdata');

                }else{

                    $restand_seconds_for =  $actual_execution_limit - ($this->sl_time_ini_process - microtime(1) );
                    $this->debbug('Limit from actual frequency  frequency  '.print_r($restand_seconds_for,1),'syncdata');

                }

                $this->debbug('Set Max execution time from cron register limit '.$restand_seconds_for,'syncdata');
    ...
    public function saveCronExecutionTime(){
        ...
        if(count($result)) {
            ...
            if(!$old_execution_time){
                ...
                $this->debbug('Compare executions of cron '.date('H:i:s',$timetosave).' Old->'.date('H:i;s',$old_execution_time) ,'autosync');
    ...
    /**
     * Function that will verify if there is need to call the synchronization again in case the frequency of cron calls is not enough
     * $force bool  for force call to cron reproduce all content
     * @throws PrestaShopDatabaseException
     * @throws PrestaShopException
     */
    public function verifyRetryCall($force = false){
       ...
       // $this->debbug('Entry to retry call ','syncdata');
        ...
        if( isset($result[0]['updated_at']) && ( ( count($result) && $register_forProcess )|| $force )) {
            ...
            $this->debbug('Execution time of cron is  '.$updated_time.' and time limit-> '.$execution_frequency_cron.' ','syncdata');
            ...
            if(($execution_frequency_cron > 0  &&  $next_sync > $if_start_now ) || $force  ){
              $restant_seconds_for_next_sync =    round($next_sync - $now_is - 10);
                ...
                $this->debbug('Execution of frequency execution of disponible_for_synchronization is '.$restant_seconds_for_next_sync.' and time limit-> '.$this->max_execution_time.' next synchronization from cron espected at '.date('d-m-Y H:i:s',$next_sync).' now with duration of this porocess if start terminate at ->'.date('d-m-Y H:i:s',$next_sync),'syncdata');
                //verify load of cpu
                $load = sys_getloadavg();
                if($load[0] > $this->cpu_max_limit_for_retry_call){
                    $this->debbug('## Warning. The call will not be executed to start the process because the cpu of the server is heavily loaded, it will try to synchronize later when the cron resumes the synchronization. cpu loaded->'.print_r($load[0],1).' limit is ->'.$this->cpu_max_limit_for_retry_call,'syncdata');
                }

                if(( $this->max_execution_time < $restant_seconds_for_next_sync || $force ) && $load[0] < $this->cpu_max_limit_for_retry_call ){

                    $url = _PS_BASE_URL_._MODULE_DIR_.'saleslayerimport/saleslayerimport-cron.php?token='.substr(Tools::encrypt('saleslayerimport'), 0, 10).'&internal=1';
                    $this->debbug('Calling execution of this syncgronization $restant_seconds_for_next_sync-> '.$restant_seconds_for_next_sync.' and time limit-> '.$this->max_execution_time.' force ->'.print_r($force,1).'Call RETRY to '.$url,'syncdata');
                    ...
                    $this->debbug('Calling this process is not necessary if the prestashop calls to cron appear in sufficient frequency for this process Or cpu is overloaded $restant_seconds_for_next_sync-> '.$restant_seconds_for_next_sync.', time limit-> '.$this->max_execution_time.' load_cpu->'.$load[0].' cpu limit stop config ->'.$this->cpu_max_limit_for_retry_call,'syncdata');

                }
            }else{

                $this->debbug(' If another process were to be executed, it would not be complete before the other. $execution_FREQUENCY_cron->'.print_r($execution_frequency_cron,1).'seconds, $next_sync ->'.print_r(date('d-m-Y H:i:s',$next_sync),1).' $if_start_now terminate at->'.print_r( date('d-m-Y H:i:s',$if_start_now),1) ,'syncdata' );
            ...
            if($force){
                ...
                $url = _PS_BASE_URL_._MODULE_DIR_.'saleslayerimport/saleslayerimport-cron.php?token='.substr(Tools::encrypt('saleslayerimport'), 0, 10).'&internal=1';
                $this->debbug('Calling execution of this syncgronization  and time limit-> '.$this->max_execution_time.' force ->'.print_r($force,1).'Call RETRY to '.$url,'syncdata');

                $this->urlSendCustomJson('GET',$url,null,false);
                return true;
            }else{
                $this->debbug('Else The call will not be made -> '.print_r($result,1).' registers for process ->'.print_r($register_forProcess,1),'syncdata');
            }
        }
    }
    ...
    public function deleteTabLink(){
        try{
            ...
            $this->debbug('## Error. There was an error in the removal of the sales layer menu.'.print_r($e->getMessage(),1),'error');
        }
    }
	...
	public function install(){
        try{
            ...
            if (!file_exists(DEBBUG_PATH_LOG)) { mkdir(DEBBUG_PATH_LOG, 0777, true); }
	...
	public function checkDB(){
        ...
        $schemaSQL_PS_Config = "CREATE TABLE IF NOT EXISTS "._DB_PREFIX_."slyr_".$this->sl_updater->table_config. " (".
            ...
            'PRIMARY KEY (`cnf_id`)'.
            ') ENGINE='.$this->sl_updater->table_engine.' ROW_FORMAT='.$this->sl_updater->table_row_format.' DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 ';

        Db::getInstance()->execute($schemaSQL_PS_Config);

		$schemaSQL_PS_SL_C_P = "CREATE TABLE IF NOT EXISTS ".$this->slyr_table." (
								`id` int(10) unsigned NOT NULL AUTO_INCREMENT,
								`ps_id` int(11) NOT NULL COMMENT '(prestashop_id| attribute_id if ps_type = product_format_value)',
								`slyr_id` bigint(20) NOT NULL COMMENT '(sales_layer_id)',
								`ps_type` varchar(32) NOT NULL COMMENT '(slCatalogue|product|product_format_field|product_format_value|combination)',
		...
		$query_read = " SELECT * FROM INFORMATION_SCHEMA.COLUMNS ".
						...
						" AND table_name = '".$this->slyr_table."' ".
		...
		if (empty($shops_info)){
	   		...
	   		$query_alter = " ALTER TABLE ".$this->slyr_table. " ADD COLUMN `shops_info` text COMMENT 'Sales Layer connectors shops'";

	   		Db::getInstance()->execute(sprintf($query_alter));

		}

		$schemaSQL_PS_SL_C_I = " CREATE TABLE IF NOT EXISTS ".$this->slyr_images_table." (
								`image_reference` varchar(255) NOT NULL COMMENT '(Sales Layer original image reference)',
		...
		$query_read = " SELECT * FROM INFORMATION_SCHEMA.COLUMNS ".
						" WHERE table_schema = '"._DB_NAME_."' ".
						" AND table_name = '".$this->slyr_images_table."' ".
		...
		if (empty($md5_image)){
	   		...
	   		$query_alter = " ALTER TABLE ".$this->slyr_images_table." ADD COLUMN `md5_image` varchar(128) NOT NULL COMMENT '(prestashop image md5)'";
	   		Db::getInstance()->execute(sprintf($query_alter));
        ...
        $schemaSQL_PS_SL_SETDATA = "CREATE TABLE IF NOT EXISTS ".$this->saleslayer_syncdata_table. " (
        ...
        if (empty($shops_info)){
            $query_alter = "ALTER TABLE "._DB_PREFIX_."slyr_".$this->sl_updater->table_config. " ADD COLUMN `auto_sync` INT(11) NOT NULL DEFAULT 0 AFTER `updater_version` ";
        ...
        if (empty($shops_info)){
            $query_alter = "ALTER TABLE "._DB_PREFIX_."slyr_".$this->sl_updater->table_config. " ADD COLUMN `last_sync` DATETIME NULL AFTER `auto_sync` ";
            Db::getInstance()->execute($query_alter);
        }
        ...
        if (empty($shops_info)){
            $query_alter = "ALTER TABLE "._DB_PREFIX_."slyr_".$this->sl_updater->table_config. " ADD COLUMN `auto_sync_hour` INT(2) NOT NULL DEFAULT 0 AFTER `last_sync` ";
            Db::getInstance()->execute($query_alter);
        }
        ...
        if (empty($shops_info)){
            $query_alter = "ALTER TABLE "._DB_PREFIX_."slyr_".$this->sl_updater->table_config. " ADD COLUMN `avoid_stock_update` INT(2) NOT NULL DEFAULT 0 AFTER `auto_sync_hour` ";
            Db::getInstance()->execute($query_alter);
        }
        ...
        if (empty($ps_variant_id)){
            ...
            $query_alter = " ALTER TABLE ".$this->slyr_images_table." ADD COLUMN `ps_product_id` BIGINT COMMENT '(prestashop ps_product_id)'";
            Db::getInstance()->execute(sprintf($query_alter));
        ...
        if (empty($ps_variant_id)){
            ...
            $query_alter = " ALTER TABLE ".$this->slyr_images_table." ADD COLUMN `ps_variant_id` varchar(1000) COMMENT '(prestashop ps_variant_id)'";
        ...
        if (empty($ps_variant_id)){
            $query_alter = " ALTER TABLE ".$this->slyr_images_table." ADD COLUMN `origin` varchar(4) COMMENT '(photo import from origin)'";
            Db::getInstance()->execute(sprintf($query_alter));
        }
	...
	public function uninstall(){
        try{
            ...
            if (!empty($connectors)){
              ...
              $this->sl_updater->delete_all(true);

            }
            $this->deleteTabLink();
            $this->deleteSlcronRegister();

            //Db::getInstance()->execute('DROP TABLE IF EXISTS '.$this->slyr_table);
            if(!parent::uninstall()){
    ...
    /**
     * Main function for Prestashop for show content in template after configuration panel in module manager ->config
     * @return string
     * @throws PrestaShopDatabaseException
     * @throws PrestaShopException
	...
	public function getContent(){
        $create_validation_table = '';
            ...
            $this->context->smarty->assign(array(
                'SLY_LOGOS_PATH'     =>$this->module_path.'config/logos/',
    ...
    public function getAllConnectors(){
        ...
        $connectors = $this->sl_updater->get_connectors_info(null, true);
        ...
        if (!empty($connectors)){
            ...
            foreach ($connectors as $conn_key => $connector) {
                unset($connectors[$conn_key]['cnf_id'],$connectors[$conn_key]['conn_schema'],$connectors[$conn_key]['data_schema'],$connectors[$conn_key]['conn_extra']);


                $conn_extra_info = $this->sl_updater->get_connector_extra_info($connector['conn_code']);
                    ...
                    if (isset($conn_extra_info['shops'])){
                        ...
                        foreach ($updated_shops as $keyUS => $updated_shop){
                            ...
                            foreach ($conn_extra_info['shops'] as $connector_shop) {
                                ...
                                if ($connector_shop['checked'] == true && $connector_shop['id_shop'] == $updated_shop['id_shop'] ){
                ...
                if (!empty($connectors[$conn_key]['shops'])||(isset($conn_extra_info['shops']) && !empty($connectors[$conn_key]['shops']) && $connectors[$conn_key]['shops'] != $conn_extra_info['shops'])  ){


                    $this->sl_updater->set_connector_extra_info($connector['conn_code'], array('shops' => $connectors[$conn_key]['shops']));

                }

                if ($connectors[$conn_key]['last_update'] != null && $connectors[$conn_key]['last_update'] != 0){

                    $connectors[$conn_key]['last_update'] = date('Y-m-d H:i:s', $connectors[$conn_key]['last_update']);
     ...
     * @return array|bool|false|mysqli_result|PDOStatement|resource|null array with all the shops info and the first checked
     * @throws PrestaShopDatabaseException
     * @throws PrestaShopException
     */
    ...
    public function writeSqlData( $sql , $params ){
    ...
    private function callIndexer(){
        ...
        $adminurl = _PS_BASE_URL_.__PS_BASE_URI__.$admin_folder.'/searchcron.php?full=1&token=' . substr(_COOKIE_KEY_, 34, 8);
     ...
     * @throws PrestaShopDatabaseException
     */
    protected function getConnectorShops($connector_id){

        $extra_info = $this->sl_updater->get_connector_extra_info($connector_id);
    ...
    /**
     * Check if in the table of crons of prestashop there is the record that executes the sales layer plugin and if it does not exist it will be created and it verifies if in the last hour it has been executed at least once.
     * @return array
     * @throws PrestaShopDatabaseException
     * @throws PrestaShopException
     */
          ...
          if(count($result)){
              ...
              $limit_time   = strtotime($result[0]['timeBD']) - (60*60);
                ...
                if($updated_time > $limit_time ){
                    ...
                    $construct_prestashop_cron_url = '*/5 * * * *  curl "'.$this->context->link->getAdminLink('AdminCronJobs',false).'&token='.Configuration::getGlobalValue('CRONJOBS_EXECUTION_TOKEN').'"';
                    $this->debbug('## Error. The activity of Prestashop cron has not been detected. Last time the SL cron needed for synchronization is executed ->'.print_r(date( 'd/m/Y H:i:s' ,$updated_time),1).'<br>'.
                        'It is strictly necessary that on your server you activate the cron job that executes the tasks of prestashop to be able to execute the automatic synchronizations of prestashop.<br>'.
                        'Add the following command to your cronjobs on your server: <br> '.$construct_prestashop_cron_url );
                    $return['stat'] = false;
                    $return['message']  = 'The activity of Prestashop cron has not been detected.<br>';
                    $return['message'] .= 'It is strictly necessary that on your server activate the cron job that executes the tasks of prestashop. To be able to execute the automatic synchronizations Sales layer plugin.<br>';
                    $return['message'] .= 'Add the following command to your cronjobs on your server: <br> '.$construct_prestashop_cron_url;

                }

          }else{
              $task_url = urlencode(_PS_BASE_URL_._MODULE_DIR_.'saleslayerimport/saleslayerimport-cron.php?token='.substr(Tools::encrypt('saleslayerimport'), 0, 10));
              $task = array(
                  'task'=> $task_url,
                  'hour'=> -1,
                  'day'=> -1,
     ...
     */
      public function testSlcronExist(){
          ...
          $query_full = 'SELECT id_cronjob,updated_at,NOW() as timeBD FROM '.$this->prestashop_cron_table.$where.' LIMIT 1';
     ...
     * @return bool
     */
      public function createSlcronRegister($config_task){
      ...
      */
      public function deleteSlcronRegister(){
            $sql_delete_query = "DELETE FROM ".$this->prestashop_cron_table." WHERE task LIKE '%saleslayerimport-cron%' ";
            $this->sl_connection_query('-',$sql_delete_query);
      }
     ...
     * @throws PrestaShopException
     */
	public function clearDeletedSlyrRegs(){ //$company_id
        $this->debbug('Reasync deleted content and remove all from sl table if not exist in prestashop','syncdata');

		$schemaCats =	" SELECT id FROM ".$this->slyr_table." AS sl ".
						" LEFT JOIN ".$this->category_table." AS c ON (c.id_category = sl.ps_id ) ".
						" WHERE  ps_type = 'slCatalogue' AND c.id_category is null ";
		$categoriesDelete = Db::getInstance()->executeS($schemaCats);
		
		if (!empty($categoriesDelete)){
		  //  $this->debbug(' delete this slCatalogue register but already deleted '.print_r($categoriesDelete,1).' sql->'.$schemaCats);
			foreach ($categoriesDelete as $categoryDelete) {
				Db::getInstance()->execute(
	                sprintf('DELETE FROM '.$this->slyr_table.' WHERE id = "%s"',
	                $categoryDelete['id']
	            ));
			}
		}

		$schemaProds =	" SELECT id FROM ".$this->slyr_table." AS sl ".
					" LEFT JOIN ".$this->product_table." AS p ON (p.id_product = sl.ps_id ) ".
					" WHERE  ps_type = 'product' AND p.id_product is null ";
		$productsDelete = Db::getInstance()->executeS($schemaProds);
					
		if (!empty($productsDelete)){
          //  $this->debbug(' delete this product register but already deleted '.print_r($productsDelete,1).' $sql->'.$schemaProds);
		...
		$schemaAttrs =	" SELECT id FROM ".$this->slyr_table." AS sl ".
					" LEFT JOIN ".$this->attribute_group_table." AS ag ON (ag.id_attribute_group = sl.ps_id ) ".
					" WHERE  ps_type = 'product_format_field' AND ag.id_attribute_group is null ";
		$attributesDelete = Db::getInstance()->executeS($schemaAttrs);
					
		if (!empty($attributesDelete)){
          //  $this->debbug(' delete this product_format_field register but already deleted '.print_r($attributesDelete,1).' $sql->'.$schemaAttrs);
        ...
        $schemaAttributesGroup =	" SELECT at.id_attribute FROM ".$this->attribute_table." AS at ".
            " LEFT JOIN ".$this->attribute_group_table." AS pa ON (pa.id_attribute_group = at.id_attribute_group ) WHERE pa.id_attribute_group is null ";

        $deleteAttributeGroup = Db::getInstance()->executeS( $schemaAttributesGroup);

        if (!empty($deleteAttributeGroup)){
            $this->debbug('## warning. Attribute values are sent to delete because their group no longer exists ->'.print_r($deleteAttributeGroup,1));
		...
		$schemaAttrVals =	" SELECT id FROM ".$this->slyr_table." AS sl ".
					" LEFT JOIN ".$this->attribute_table." AS a ON (a.id_attribute = sl.ps_id ) ".
					" WHERE  ps_type = 'product_format_value' AND a.id_attribute is null ";
		$attributeValuesDelete = Db::getInstance()->executeS($schemaAttrVals);
					
		if (!empty($attributeValuesDelete)){
          //  $this->debbug(' delete this product_format_value register but already deleted '.print_r($attributeValuesDelete,1).' sql->'.$schemaAttrVals);
		...
		$schemaFeatures =	" SELECT id FROM ".$this->slyr_table." AS sl ".
									" LEFT JOIN ".$this->product_attribute_table." AS pa ON (pa.id_product_attribute = sl.ps_id ) ".
									" WHERE  ps_type = 'combination' AND ( pa.id_product_attribute is null OR pa.id_product = 0) ";
		$featuresDelete = Db::getInstance()->executeS($schemaFeatures);
					
		if (!empty($featuresDelete)){
          //  $this->debbug(' delete this combination register but already deleted '.print_r($featuresDelete,1).' sql->'.$schemaFeatures);
        ...
        $schemaImages =	" SELECT sl.id_image FROM ".$this->slyr_images_table." AS sl ".
            " LEFT JOIN ".$this->image_shop_table." AS pa ON (pa.id_image = sl.id_image ) WHERE pa.id_image is null ";

        $deleteImages = Db::getInstance()->executeS($schemaImages);

        if (!empty($deleteImages)){
           // $this->debbug('Enviando imagenes para eliminar de tabla SLYR que ya no existen en la tabla de imagenes de prestashop $deleteImages->'.print_r($deleteImages,1).' query->'.print_r($schemaImages,1));
            foreach ($deleteImages as $ImageforDelete) {

                Db::getInstance()->execute( sprintf('DELETE FROM '.$this->slyr_images_table.' WHERE id_image = "%s"',
     ...
     * @return void
     */
    public function insert_syncdata_sql($force_insert = false){
        if (!empty($this->sql_to_insert) && (count($this->sql_to_insert) >= $this->sql_insert_limit || $force_insert)){
            ...
            try{
                ...
                $sql_query_to_insert = " INSERT INTO ".$this->saleslayer_syncdata_table.
                    ...
                    $sql_to_insert;
                $this->sl_connection_query('insert', $sql_query_to_insert);
     ...
     */
    public function sl_connection_query($type, $query, $params = array()){
        $resultado = false;
        try{
            ...
            if (!empty($params)){
                ...
                $this->debbug('## Error. SL SQL type: '.$type.' - query: '.$query.' - params: '.print_r($params,1));
    ...
    public function get_conn_field($connector_id, $field_name) {
        ...
        if($connector_id != null){
            ...
            $sql= "SELECT ".$field_name." FROM "._DB_PREFIX_."slyr_".$this->sl_updater->table_config." WHERE `conn_code`='".addslashes($connector_id)."' limit 1 ;";
            $return =  $this->sl_connection_query('read',$sql);
    ...
    /**
     * Repair structure of image. Remove images that are not linked with other tables so that prestashop can assign new images.
     * @param $id_product
     * @return bool
     * @throws PrestaShopDatabaseException
     * @throws PrestaShopException
     */
    public function repairImageStructureOfProduct( $id_product){
        $id_product = (string) $id_product;
        $ids_images_all = array();
        $counter = 0;
        $this->debbug('Entry to repair images of product with ps_id -> '.print_r($id_product,1),'syncdata');

        $ids_images = Db::getInstance()->executeS( sprintf('SELECT id_image FROM ' . $this->image_shop_table . ' WHERE id_product = "%s" GROUP BY id_image ',$id_product) );
        $this->debbug('id_images from '.$this->image_shop_table.' -> '.print_r($ids_images,1),'syncdata');

        $ids_images_image = Db::getInstance()->executeS( sprintf('SELECT id_image  FROM ' . $this->image_table . ' WHERE id_product = "%s" GROUP BY id_image ',$id_product) );
        $this->debbug('id_images from '.$this->image_table.' -> '.print_r($ids_images_image,1),'syncdata');


        $this->debbug('before merge  '.print_r($ids_images,1).' with ->'.print_r($ids_images_image,1),'syncdata');
        try{
            if(count($ids_images)){
        ...
        if(count($ids_images_all)){
            foreach($ids_images_all as $id_image){
                ...
                try {
                    $ps_images = Db::getInstance()->executeS(sprintf('SELECT id_image FROM ' . $this->image_table . ' WHERE id_image = "%s" ', $id_image));
                    if (empty($ps_images)) {
                        $test_ok_ps_image = false;
                    }
                }catch(Exception $e){
                    $this->debbug('## Error. Selecting images of id_image -> '.print_r($id_image,1).' occured->'.print_r($e->getMessage(),1),'syncdata');
                }
                /*test if exist ps_image_lang registers*/
                try{
                    $ps_images_lang = Db::getInstance()->executeS( sprintf('SELECT id_image FROM ' . $this->image_lang_table . ' WHERE id_image = "%s" ',$id_image) );
                    if(empty($ps_images_lang)){
                        $test_ok_image_lang_table = false;
                    }
                }catch(Exception $e){
                    $this->debbug('## Error. Selecting images of id_image lang table -> '.print_r($id_image,1).' occured->'.print_r($e->getMessage(),1),'syncdata');
                }
                try{
                    /*test if exist ps_image_shop registers*/
                    $ps_images_shop = Db::getInstance()->executeS( sprintf('SELECT id_image FROM ' . $this->image_shop_table . ' WHERE id_image = "%s" ',$id_image) );
                ...
                if($test_ok_ps_image && $test_ok_image_lang_table && $test_ok_image_shop){
                    ...
                    $this->debbug('Image '.print_r($id_image,1).' is  corrupt how to a delete it for posibility to create it: test_ok_ps_image->'.print_r($test_ok_ps_image,1).',  test_ok_image_lang_table->'.print_r($test_ok_image_lang_table,1).', test_ok_image_shop->'.print_r($test_ok_image_shop,1),'syncdata');
                    ...
                    if($delete_return){
                        ...
                        $this->debbug('Image deleting '.print_r($id_image,1).' is ok from new Image() function','syncdata');
                        Db::getInstance()->execute(sprintf("DELETE FROM ".$this->slyr_images_table." WHERE id_image = '%s' ",$id_image));

                    }else{

                        $this->debbug('Image deleting '.print_r($id_image,1).' else from new Image how to a delete from BD','syncdata');
                        Db::getInstance()->execute(sprintf("DELETE FROM ".$this->image_table."       WHERE id_image = '%s' ",$id_image));
                        Db::getInstance()->execute(sprintf("DELETE FROM ".$this->image_lang_table."  WHERE id_image = '%s' ",$id_image));
                        Db::getInstance()->execute(sprintf("DELETE FROM ".$this->image_shop_table."  WHERE id_image = '%s' ",$id_image));
                        Db::getInstance()->execute(sprintf("DELETE FROM ".$this->slyr_images_table." WHERE id_image = '%s' ",$id_image));
        ...
        $this->debbug('## Info. Structure Repaired Complete! '.($counter > 0  ?'It is likely that the problem was solved.':' No image was found that prevents insertion.'),'syncdata');
    }
                ...
                if ($connectors_to_check){
                    ...
                    foreach ($connectors_to_check as $connector) {
                       ...
                       // $data_return = $this->store_sync_data($connector_id, $last_sync);
                        $data_return =  $sync_libs->store_sync_data($connector_id,$last_sync);
    ...
    public function checkRegistersForProccess($return_num = false){
    ...
    public function sync_data_connectors(){
        ...
        $this->sl_catalogues = new sl_catalogues();
        $this->sl_products   = new sl_products();
        $this->sl_variants   = new sl_variants();
        $processed           = array();
        ...
        $load = sys_getloadavg();
            ...
            if(($this->cpu_max_limit_for_retry_call * 2) < $load[0]){
                $this->debbug('## Warning. at:'.date('H:i:s').' We have detected that the cpu is overloaded, The saturation limit of cpu doubled. Cpu load->'.print_r($load[0],1).' limit ->'.$this->cpu_max_limit_for_retry_call. '. It\'s possible that that\'s why the synchronization will go much slower. We have reduced the number of insertions per process to 1. ','syncdata');
                $this->sql_insert_limit = 1;
                //return false;
            }elseif($load[0] > $this->cpu_max_limit_for_retry_call){
                $this->debbug('## Warning. at:'.date('H:i:s').' We have detected that the cpu is overloaded, we will try to postpone this synchronization for a 10 seconds so as not to saturate your server any more. Cpu load->'.print_r($load[0],1).' limit ->'.$this->cpu_max_limit_for_retry_call. ' send sleep. We have reduced the number of insertions per process to 3. ','syncdata');
        ...
        if (!$this->end_process){
            ...
            try {
                ...
                $items_to_delete = $this->sl_connection_query('read'," SELECT * FROM ".$this->saleslayer_syncdata_table." WHERE sync_type = 'delete' ORDER BY item_type ASC, sync_tries ASC, id ASC ");
                $this->reasignMemoryNeccesary();
                if (!empty($items_to_delete)){
                         ...
                         //   $this->store_view_ids = $sync_params['conn_params']['store_view_ids'];
                            ...
                            switch ($item_to_delete['item_type']) {
                                case 'category':
                                    ...
                                    $result_delete = $this->sl_catalogues->deleteCategory($sl_id, $this->comp_id,$this->conector_shops_ids);
                                    break;

                                case 'product':
                                    $result_delete = $this->sl_products->deleteProduct($sl_id, $this->comp_id,$this->conector_shops_ids);
                                    break;

                                case 'product_format':
                                    $result_delete = $this->sl_variants->deleteVariant($sl_id, $this->comp_id,$this->conector_shops_ids);
                                    break;
                    ...
                    $this->debbug('Lanzando regenearacion de tablas despues de delete regenerateEntireNtree','syncdata');
            ...
            foreach ($indexes as $index) {
                if($index == 'product' && $categories_clear ){
                    ...
                    try{
                        ...
                        $this->debbug('## Error. In reorganize Categories after Update '.$e->getMessage().' line->'.$e->getLine(),'syncdata');
                    }
                ...
                do{
                    ...
                    $items_to_update = $this->sl_connection_query('read'," SELECT * FROM ".$this->saleslayer_syncdata_table." WHERE sync_type = 'update' and item_type = '".$index."' ORDER BY item_type ASC, sync_tries ASC, id ASC LIMIT ".$this->sql_insert_limit);

                    if (!empty($items_to_update)){


                       $processed_ithems = $this->update_items($items_to_update);
                       unset($items_to_update);
                        ...
                        $sql_check_try++;
                        $this->debbug('Break but is empty','syncdata');
                        unset($items_to_update);
                        break;
    ...
    private function check_process_time(){
        ...
        if ($current_process_time >= $this->max_execution_time){
            $this->debbug('The predefined time for the synchronization has been exceeded in the next opportunity, the synchronization will be calibrated and the synchronization will continue in its next process.->'.$current_process_time.' seconds max_execution_time:'.ini_get('max_execution_time') );
    ...
    private function check_sql_items_delete($force_delete = false){
        ...
        if (count($this->sql_items_delete) >= $this->sql_insert_limit || ($force_delete && count($this->sql_items_delete) > 0)){
    ...
    private function disable_sync_data_flag(){
        ...
        $current_flag = $this->sl_connection_query('read'," SELECT * FROM ".$this->saleslayer_syncdata_flag_table." ORDER BY id DESC LIMIT 1");
        ...
        if ($this->checkRegistersForProccess()){
            ...
            if (!empty($current_flag)){
                ...
                if ($current_flag['syncdata_pid'] == 0){
                    ...
                    $this->sl_connection_query('read',$sl_query_flag_to_update);
    ...
    public function has_pid_alive ($pid) {
        ...
        if ($pid) {
            try{
                ...
                $this->debbug('## Error. The plugin has tried to release a stuck process but without success ->'.$e->getMessage(),'syncdata');
            }
        }
    ...
    private function update_items($items_to_update){
        ...
        foreach ($items_to_update as $item_to_update) {
            ...
            if ($this->end_process){
                ...
                if ( isset($item_to_update['sync_params'])&& !empty($item_to_update['sync_params']) &&  $item_to_update['sync_params'] != ''){
                ...
                if ($item_data == ''){
                    ...
                    switch ($item_to_update['item_type']) {
                        case 'category':
                            $this->sl_catalogues->loadCategoryImageSchema($sync_params['conn_params']['data_schema']);
                            ...
                            try{
                                ...
                                $this->debbug('## Error. Synchronizing category '.print_r($e->getMessage(),1),'syncdata');
                            }

                           // $result_update = $this->sync_stored_category($item_data);
                            $this->debbug(' >> Category synchronization finished << ','syncdata');
                            $this->debbug('#### time_sync_stored_category: '.$item_data['sync_data']['ID'].'->'.(microtime(1) - $time_ini_sync_stored_category).' seconds.', 'timer');
                            break;

                        case 'product':
                            $this->debbug(' >> Product start << ','syncdata');
                            $this->sl_products->loadProductImageSchema($sync_params['conn_params']['data_schema']);
                            $this->load_connector_Accesories();
                            ...
                            try{
                                ...
                                if(isset($result_update_array['additional_output']['product_accessories']) && !empty($result_update_array['additional_output']['product_accessories'])){

                                     $this->product_accessories[$result_update_array['additional_output']['product_psid']] =  $result_update_array['additional_output']['product_accessories'];

                                }

                                 $this->save_stat_Accesories();

                            }catch(Exception $e){
                                $result_update = 'item_not_updated';
                                    $this->debbug('## Error. Synchronizing product '.print_r($e->getMessage(),1).' trace->'.print_r($e->getTrace(),1).' line->'.$e->getLine(),'syncdata');
                            }

                           // $result_update = $this->sync_stored_product($item_data);
                            $this->debbug(' >> Product synchronization finished << ','syncdata');
                            $this->debbug('#### time_sync_stored_product: '.$item_data['sync_data']['ID'].'->'.(microtime(1) - $time_ini_sync_stored_product).' seconds.', 'timer');
                            break;
                        ...
                        case 'product_format':
                            ...
                            try{
                                ...
                                $result_update = 'item_not_updated';
                                $this->debbug('## Error. Synchronizing Variant '.print_r($e->getMessage(),1).' trace->'.print_r($e->getTrace(),1).' line->'.$e->getLine(),'syncdata');
                            }


                            //$result_update = $this->sync_stored_product_format($item_data);
                            $this->debbug(' >> Format synchronization finished << ','syncdata');
                            $this->debbug('#### time_sync_stored_product_format: '.$item_data['sync_data']['ID'].'-> '.(microtime(1) - $time_ini_sync_stored_product_format).' seconds.', 'timer');
                            break;
                        ...
                        case 'accessories':
                            ...
                            try{
                                ...
                                $result_update = 'item_not_updated';
                                $this->debbug('## Error. Synchronizing accessories '.print_r($e->getMessage(),1));

                            }

                            $this->debbug(' >> Product Accesories Fin<< ');
                            $this->debbug('#### time_sync_stored_product_accessories: '.(microtime(1) - $time_ini_sync_stored_product_accesories).' seconds.', 'timer');
                            break;
                        /*
                ...
                switch ($result_update) {
                    case 'item_not_updated':
                        ...
                        if ($sync_tries > 2  ){
                            ...
                            $this->debbug('## Error. item_not_updated: : '.print_R($item_to_update,1), 'syncdata');
                                $sql_update = " UPDATE ".$this->saleslayer_syncdata_table.
                                    " SET sync_tries = ".$sync_tries.
    ...
    public function load_connector_Accesories(){

        $item_type = 'accessories';
        $sync_type = 'update';
        $sql_sel = " SELECT * FROM ".$this->saleslayer_syncdata_table." WHERE sync_type = '$sync_type' AND item_type = '$item_type' LIMIT 1  ";
        $res  = $this->sl_connection_query('read',$sql_sel);
    ...
    public function set_connector_Data ($connector_id,$field_name,$field_value ) {

        if($connector_id != null && $field_name != null  ){
            $sql_FUPD = "UPDATE "._DB_PREFIX_."slyr_".$this->sl_updater->table_config ." SET ".$field_name." = '$field_value' WHERE conn_code = '".addslashes($connector_id)."'  limit 1 ";
    ...
    public function save_stat_Accesories():void{
        ...
        if(count($this->product_accessories)){
            ...
            if(!$res){
              ...
              $result =   $this->sl_connection_query('-', $sql_query_to_insert);

            }else{

                $sql_query_to_insert = " UPDATE ".$this->saleslayer_syncdata_table.
                    " SET item_data =  '".addslashes($item_data_to_insert)."' WHERE sync_type = '$sync_type' AND item_type = '$item_type' ";
    ...
    public function syncAccesories(){
        ...
        if (!empty($this->product_accessories)){
            ...
            foreach ($this->product_accessories as $product_id => $product_accessories) {
                ...
                if (!empty($product_accessories)){
                    ...
                    foreach ($product_accessories as $product_accessory_reference) {
                        ...
                        // $product_accessory_reference = trim(preg_replace('/[^A-Za-z0-9\-]/', ' ', $product_accessory_reference));
                        $product_accessory_reference = $this->sl_validate_reference($product_accessory_reference);

                        //find product with the same reference
                        $schemaRef = "SELECT id_product FROM ".$this->product_table." WHERE reference = '".$product_accessory_reference."'";
        ...
        if($data != null && is_array($data)){
            foreach ($data as $configname => $save_value){
               // $this->debbug('New configuration to save '.$configname.'   value ->'.$save_value,'syncdata');
                ...
                $res  = $this->sl_connection_query('read',$sql_sel);
                if(!empty($res)){
                    $sql_sel = "UPDATE ".$this->saleslayer_aditional_config." SET  save_value = '".$save_value."'  WHERE id_config = '".$res[0]['id_config']."' Limit 1  ";
                    $res  = $this->sl_connection_query('-',$sql_sel);
                }else{
                    $sql_sel = "INSERT INTO  ".$this->saleslayer_aditional_config." ( configname , save_value ) VALUES ('".$configname."','".$save_value."') ";
                    $res  = $this->sl_connection_query('insert',$sql_sel);
                }
            }
        }
    }
    ...
    public function getConfiguration($configuration_name){
        $sql_sel = "SELECT * FROM ".$this->saleslayer_aditional_config." WHERE configname = '$configuration_name'  ";
        $res  = $this->sl_connection_query('read',$sql_sel);
        if(!empty($res)){
     ...
     * @return bool
     */
    public function deleteConfiguration($configuration_name){
        $sql_sel = "DELETE FROM ".$this->saleslayer_aditional_config." WHERE configname = '$configuration_name'  Limit 1  ";
        $res  = $this->sl_connection_query('-',$sql_sel);
        if($res){
            return true;
        }
     ...
     * Print debbug
     */
    public function print_debug_content(){
        foreach($this->debbug_occurence as  $debugline){
     ...
     * Clear debug content
     */
    public function clear_debug_content(){
        $this->debbug_occurence = array();
    }
    /**
     * Delete any files older than  num defined in $this->logfile_delete_days days
     */
    public function delete_old_debug_files(){
    ...
    /**
     * Before sensitive processes to use a lot of memory, we calculate current usage and multiply it to the amount needed to finish the job.
     */
    public function reasignMemoryNeccesary(){
        ...
        if($memory_remaining < 100){
            $this->debbug('The synchronization does not have many mb left to work, it will be assigned more ->'.$memory_multiply,'syncdata');
            $memory_multiply = $memory_multiply * 3;
        }

        $this->debbug('Actual Memory limit ->'.$actual_limit.' In Use->'.$mem.' memory recomended ->'.$memory_multiply,'syncdata');
     ...
     * @return void | bool
     */
    public function debbug($mensaje ='', $type_file = false, $force_print = false){
        ...
        if(!$force_print){
            ...
            if (isset($func[1]['function']) and !in_array($func[1]['function'], ['debbug', 'debbug_error', 'include', 'require', 'query_db', 'halt'])) {
                // alineacion de linias
                $line_chars = 5;
                $separator = '';
                $discount_chart = strlen($func[0]['line']);
                ...
                $separator_function = '';
                $discount_chart_function = strlen($func[1]['function']);
                ...
                $mensaje = 'Line:' . $func[0]['line'] . $separator . '> ' . $func[1]['function'] . '()' . $separator_function . '> ' . trim($mensaje);
            }
            if ($this->debugmode > 2 || $error_content) {
                ...
                if (isset($func[1]['file'])) {
                    ...
                    $discount_chart_function_file = strlen('[' . $file_last . ']');
                    if ($discount_chart_function_file < $line_chars_functions_file) {
                        for (; $discount_chart_function_file < $line_chars_functions_file; $discount_chart_function_file++) {
                            $separator_function_file .= ' ';
                        }
                    }
            ...
            if( !$this->debugmode && $error_content && !$force_print ){
                ...
                $this->debbug_occurence[]= array($mensaje,$type_file);
                $this->print_debug_content();
                return false;
          ...
          //  $msg = $msg.PHP_EOL;
            ...
            if (DEBBUG_PATH_LOG) {
                ...
                if($this->debugmode || $error_content || $force_print ){
                   ...
                   $this->debbug_occurence[]= array("$msg\r\n",$type_file);

                   return false;
                }


                    if ($new_file|| !$result || !is_writable($file) ){ chmod($file, 0777);
                        if(!$result){
                            error_log("$msg\r\n",3,$file);
                        }
                    }
